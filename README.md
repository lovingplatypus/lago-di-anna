# Per Anna 🕊️ + Lago di Anna 🌸

Two connected pieces, deployed as one GitHub Pages site:

1. **`index.html` — the letter.** The first thing Anna sees: an airmail
   envelope she flips and unseals, then a scrolling story (the mixer, the
   1 AM message, coffee at Kinship, the globe, the photo wall, the birthday
   note, the countdown). The "Press to play" ticket at the end links to the
   game at `./game/`.
...
```
index.html         ← the letter / landing page (plain, self-contained HTML+JS)
Per Anna.dc.html   ← original claude.ai export of the letter — REFERENCE ONLY:
                     ...
                     not run by itself. index.html is the converted, working
                     copy; make text edits in index.html.
...
Dates baked into the letter (edit in `index.html` if plans change):
